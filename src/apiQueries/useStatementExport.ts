  ...
  const mutation = useMutation<void, AppError, StatementQueryParams>({
    mutationFn: (params: StatementQueryParams): Promise<void> =>
      new Promise<void>((resolve, reject) => {
        ...
        // fetchApi with customCallback returns void | Promise<void>
        // We handle the response in the callback and manage the promise manually
        const fetchResult = fetchApi(
          url,
          {},
          {
            customCallback: (response: Response) => {
              return handleStatementExportResponse(response, params, resolve, reject);
            },
          },
        );

        // Handle the case where fetchApi returns a Promise (though customCallback should handle it)
